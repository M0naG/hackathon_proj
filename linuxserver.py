...
class ClientThread(Thread): 
    ...
    def __init__(self,conn,ip,port): 
        ...
        self.client_name = ""
        self.count = 0
        print ("[+] New server socket thread started for " + ip + ":" + str(port)) #to remove
 
    def run(self): 
        #while True : 
            data = conn.recv(2048).decode()
            print("Server received client name:", data) #to remove
...
while True:
    ...
    print("Server started, listening on IP address "+udp_ip) #to change
    timeout = 10
    ...
    while time.time() < timeout_start + timeout:  
        try:
            ...
            print ("Multithreaded Python server : Waiting for connections from TCP clients..." ) #to remove
    ...
    print("time over") #to remove
    game()

    again = input("do you want another round? y for yes, anything else for no: ") 
